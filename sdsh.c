 ...
 *  Features
 *  --------
 *  • Colorful ANSI prompt:  [sdsh] user@machine:cwd$
 *  • Command translation:
 *      File ops:    list→ls  listdat→lsblk  newfolder→mkdir  nuke→rm
 *                   clone→cp  shift→mv  peek→cat  spark→touch
...
#define COL_GREEN        "\033[38;5;82m"   /* bright green  – [sdsh] tag      */
#define COL_CYAN         "\033[38;5;87m"   /* sky cyan      – user@host       */
#define COL_YELLOW       "\033[38;5;220m"  /* golden yellow – current dir     */
 ...
 * sdsh_print_prompt
 * -----------------
 * Prints:  [sdsh] user@hostname:cwd$
 * Each segment has its own ANSI color.
 */
static void sdsh_print_prompt(void)
{
    /* ── Username ───────────────────────────────────────────────────────── */
    ...
    /* ── Hostname (first component only, e.g. "neon-box") ──────────────── */
    char hostname[256] = "localhost";
    gethostname(hostname, sizeof(hostname));
    /* Truncate at the first dot to get the short hostname */
    ...
    /* ── Render prompt ──────────────────────────────────────────────────── */
    printf(COL_BOLD COL_GREEN "[sdsh]" COL_RESET " "
           COL_BOLD COL_CYAN  "%s@%s" COL_RESET ":"
           COL_BOLD COL_YELLOW "%s" COL_RESET "$ ",
...
static void sdsh_print_about(void)
{
    printf(COL_MAGENTA COL_BOLD
        ...
        "   ███████║██████╔╝███████║██║  ██║\n"
        "   ╚══════╝╚═════╝ ╚══════╝╚═╝  ╚═╝\n"
        "      Super Duper Shell — v" SDSH_VERSION "\n"
        COL_RESET
        COL_BLUE
        "\n"
...
static const char *sdsh_translate(const char *cmd)
{
     ...
     *    listdat   → lsblk       (block device listing)
     *    newfolder → mkdir        (make directory)
     *    nuke      → rm           (remove files/dirs)
     *    clone     → cp           (copy files)
     *    shift     → mv           (move / rename)
     *    peek      → cat          (view file contents)
     *    spark     → touch        (create blank file)
     *
     *  SYSTEM & APP MANAGEMENT
